  ...
  <div class="item_detail" id="cityFullScreen" ref="cityFullScreen">
    <div>
      <div :key="index" v-for="(item, index) in cityList" ref="allCityBar">
        ...
        <div v-for="(list, key) in item.content" :key="key">
          <div style="height: 40px;line-height: 40px;">{{list.name}}</div>
        </div>
      </div>
    </div>
    <div class="selectCity_alphabet">
      <ul v-for="(item, index) in leftNav" :key="index">
        <li
          :class="index === curCityListIndex ? 'fastSelActive': ''"
          @click="goAnchor(item)"
        >{{item.name}}</li>
      </ul>
    </div>
...
export default {
  ...
  mounted() {
    this.calculateHeight();
    this.$refs.cityFullScreen.addEventListener(
      "scroll",
      this.handleScroll,
      true
    ); // 监听（绑定）滚轮滚动事件
